...
/*
// Friendship Talks
// Talk 1 - Thoughts on the City
CHAIN IF ~Global("BFSMkSoATalk1","GLOBAL",2)~ THEN BFSMKJ BFSMk2.CityThoughts
~Too-talls stack stones high and call it city.~
= ~Seems for some it offers opportunity. For others it's a safer cage.~
= ~*sigh* Hard to tell which it is for me yet.~
END
++ ~Not every city is like Athkatla. There are others with kinder, fairer rulers.~ EXTERN BFSMKJ BFSMk2.KinderCities
++ ~Every city holds its dangers. That doesn't make it a prison.~ EXTERN BFSMKJ BFSMk2.NotAPrison
+ ~Class(Player1,THIEF_ALL)~ + ~If it's a cage, I'm more than capable of picking the lock.~ EXTERN BFSMKJ BFSMk2.PickTheLock
++ ~I prefer the wilds to stone walls and crowds.~ EXTERN BFSMKJ BFSMk2.PreferWild
++ ~You don't have to decide yet. We're just passing through.~ EXTERN BFSMKJ BFSMk2.PassingThrough

CHAIN BFSMKJ BFSMk2.KinderCities
= ~Doesn't matter who leads. Someone will always be looked down at like vermin.~
END
IF ~~ EXTERN BFSMKJ BFSMk2.WrapUp

CHAIN BFSMKJ BFSMk2.NotAPrison
~Maybe. But walls don't just keep danger out. They keep folk in~
= ~In the wild, danger's honest. Here it smiles before ensnaring you.~
END
IF ~~ EXTERN BFSMKJ BFSMk2.WrapUp

CHAIN BFSMKJ BFS.SoA.PickTheLock
~Always good if you can make your own exit.~
END
IF ~~ EXTERN BFSMKJ BFSMk2.WrapUp

CHAIN BFSMKJ BFSMk2.PreferWild
~Out there, everything wants to eat you. In here, they want to use you.~
= ~Least a roof keeps the rain off. Guess I'll choose cold stone over mud.~
END
IF ~~ EXTERN BFSMKJ BFSMk2.WrapUp

CHAIN BFSMKJ BFSMk2.PassingThrough
~That's what I used to tell myself about every place.~
= ~Hard to keep walking forever, though. Even goblins need somewhere to stand.~
END
IF ~~ EXTERN BFSMK25J BFSMk2.WrapUp

CHAIN BFSMKJ BFSMk2.WrapUp
~Just need time, I think. To learn the smells. The rhythms. Figure out where I fit...if I fit.~
DO ~SetGlobal("BFSMkSoATalk1","GLOBAL",3)~
EXIT */
